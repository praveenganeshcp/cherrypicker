...
export class CreateCherrypickRequestUsecase {
    ...
    async execute(input: CreateCherrypickRequestUsecaseInput): Promise<{id: number}> {
        ...
        const createdRequest: CherrypickRequestEntity = await this.createRequest(input);
        this.logger.log(`New cherrypick request created ${createdRequest.id.toString()}`);
        ...
        this.emailNotificationService.notify({
            ...
            cta: {
                label: "Approve",
                link: `${this.appConfiguration.FE_HOST_ADDRESS}/app/cherrypick-requests/${createdRequest.id.toString()}/approve`,
    ...
    private async createRequest(input: CreateCherrypickRequestUsecaseInput): Promise<CherrypickRequestEntity> {
    ...
    private async saveRequestCommits(cherrypickRequest: CherrypickRequestEntity, commits: CreateCherrypickRequestUsecaseInput['commits']) {
        const requestCommits: Omit<CherrypickCommitEntity, "id">[] = commits.map(commit => {
