...
export class GithubService {
  ...
  async getAccessToken(exchangeCode: string): Promise<string> {
    ...
    const response = await axios.post<{ access_token: string }>(
      `${this.GITHUB_AUTH_HOST}/login/oauth/access_token`,
      {
  ...
  async getCommits(
    ...
    const response = await axios.get<BEGithubCommit[]>(
      `${this.GITHUB_API_HOST}/repos/${loginName}/${repoName}/commits`,
      {
        headers: {
          Authorization: `Bearer ${accessToken}`,
  ...
  async triggerWorkflow(
    ...
    this.logger.log(
      `Initiating cherrypick workflow with commits ${commits.join(
        ","
      )} in ${targetBranch} branch for request ${requestId}`
    );
    const response = await axios.post(
      `${this.GITHUB_API_HOST}/repos/praveenganeshcp/hello_world/actions/workflows/simpewf.yaml/dispatches`,
      {
        ref: targetBranch,
        inputs: {
