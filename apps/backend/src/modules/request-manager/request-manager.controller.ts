...
} from "@cherrypicker/request-manager-be";
import {
  Body,
  Controller,
  Get,
...
import {
  AddVCSRepoDTO,
  CreateCherrypickRequestDTO,
} from "./request-manager-dto";
...
export class RequestManagerController {
  ...
  fetchRepoCommits(
    ...
    this.logger.log(
      `Fetching commits in ${user.subjectLogin}/${repoName} repo`
    );
  ...
  createCherrypickRequest(
    @Body() createCherrypickRequestDTO: CreateCherrypickRequestDTO,
    @AuthUser() user: UserEntity
  ) {
